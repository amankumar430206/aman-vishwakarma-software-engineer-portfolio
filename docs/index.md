---
layout: home

hero:
  name: Aman Vishwakarma
  text: Software Engineer
  tagline: >
    Building high-scale fintech & cloud systems with React, TypeScript,
    Microservices, and AWS.
  image:
    src: /hero.jpeg
    alt: Aman Vishwakarma
    width: 1000
    height: 1000
  actions:
    - theme: brand
      text: Download Resume
      link: /resume.pdf

    - theme: alt
      text: View Projects
      link: /projects

    - theme: alt
      text: GitHub
      link: https://github.com/amankumar430206

    - theme: alt
      text: LinkedIn
      link: https://www.linkedin.com/in/aman-vishwakarma/

# features:
#   - icon: 🚀
#     title: Fintech at Scale
#     details: >
#       Led cross-border payments, neobanking, card processors, and expense
#       systems handling ~1M transactions per day.

#   - icon: ⚡
#     title: Frontend Architecture
#     details: >
#       Expert in React, Next.js, TypeScript, and micro-frontends with a
#       performance-first mindset.

#   - icon: ☁️
#     title: Cloud & DevOps
#     details: >
#       AWS-native systems using S3, CloudFront, ELB, ASG, CI/CD pipelines,
#       Docker, and Kubernetes.

#   - icon: 🧠
#     title: Ownership & Impact
#     details: >
#       Migrated legacy systems, reduced deployment time by 80%, and shipped
#       production-grade platforms end-to-end.
---

<p class="live-status">
  <span class="dot"></span>
  <strong>Currently building:</strong>
  <span id="live-status-text"></span>
</p>

<p class="hero-greeting" id="hero-greeting"></p>
<div class="metrics-strip">
  <div>
    <strong>4+ yrs</strong>
    <span>Experience</span>
  </div>
  <div>
    <strong>~1M/day</strong>
    <span>Transactions</span>
  </div>
  <div>
    <strong>80%</strong>
    <span>Faster Deploys</span>
  </div>
  <div>
    <strong>Multi-Region</strong>
    <span>Scale</span>
  </div>
</div>

### 👋 About Me

I’m a **Software Engineer** building
**fintech and enterprise-scale systems**.

I specialize in **frontend-heavy full-stack development**, owning products
from architecture and UI performance to cloud infrastructure and CI/CD.
Most of my work lives in **payments, neobanking, cross-border finance, and
high-traffic platforms**.

I enjoy solving hard problems, simplifying complex systems, and building
software that scales reliably.

<div class="hero-divider"></div>

### 🧩 Tech Snapshot

- **Frontend:** React, Next.js, TypeScript, Vue, Micro-frontends
- **Backend:** Node.js, REST APIs, Microservices
- **Cloud:** AWS (EC2, S3, CloudFront, ELB, ASG, Lambda, ECS)
- **DevOps:** Docker, Kubernetes, CI/CD (CodeBuild, CodePipeline)
- **Domains:** Fintech, Neobanking, Payments, Enterprise Systems

<div class="hero-divider"></div>

### 💼 Currently

- Sr. Software Engineer @ **Stackintel**
- Building **cross-border finance & neobanking platforms**
- Open to **Senior Frontend / Full-Stack / Fintech roles**

<div class="hero-divider"></div>

### 📬 Let’s Connect

<p class="availability-badge">
  🟢 Open to Senior Frontend / Full-Stack Roles
</p>

📧 **Email:** aman.vishwakarma.dev@gmail.com  
💻 **GitHub:** https://github.com/amankumar430206  
🔗 **LinkedIn:** https://www.linkedin.com/in/aman-vishwakarma/

---
